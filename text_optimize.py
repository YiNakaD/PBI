...
class Text_Attacker:
    ...
    def attack(self, text_prompt_template, offset, batch_size=8, num_iter=2000):
        ...
        trigger_token_length = 10  # equivalent to
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            for token_to_flip in range(0, trigger_token_length):  # for each token in the trigger
                ...
                loss_jail = -sum(res_lis)
                final_loss = target_loss.item() + loss_jail
                ###

                loss = target_loss  # target_loss  # to minimize
                loss.backward()

                print('[adv_prompt]', adv_prompt)
                print("target_loss: %f" % (target_loss.item()))
                print('perspective: %f' % (-loss_jail))
                print('>>> response: %s' % response)
                self.loss_buffer.append(-loss_jail)
    ...
    def attack_loss(self, prompts, targets):
        ...
        assert len(context_embs) == len(
            targets), "Unmathced batch size of prompts and targets, the length of context_embs is %d, the length of targets is %d" % (
